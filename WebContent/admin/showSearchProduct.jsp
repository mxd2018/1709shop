				...
				<td colspan="7">
					...
					<ul id="admin_ul">
						<c:if test="${pagebean.currentPage!=1 }">
							<li><a href="${pageContext.request.contextPath}/adminSearchProductServlet2?currentPage=${pagebean.currentPage-1}">上一页</a></li>
						</c:if>
						<c:if test="${pagebean.currentPage==1 }">
							<li>上一页</li>
						</c:if>
						...
						<c:forEach begin="1" end="${pagebean.totalPage }" var="page">
							<c:if test="${page!=pagebean.currentPage }">
								<li>
								<a href="${pageContext.request.contextPath}/adminSearchProductServlet2?currentPage=${page}">${page}</a>&nbsp;&nbsp;&nbsp;
								</li>
							</c:if>
							<c:if test="${page==pagebean.currentPage }">
						...
						<c:if test="${pageBean.currentPage!=pageBean.totalPage }">
							<li><a href="${pageContext.request.contextPath}/adminSearchProductServlet2?currentPage=${pageBean.currentPage+1}">下一页</a></li>
						</c:if>
						<c:if test="${pageBean.currentPage==pageBean.totalPage }">
							<li>下一页</li>
						</c:if>
